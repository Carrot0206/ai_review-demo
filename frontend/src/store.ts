...
interface AppState {
  ...
  setShowDeduped: (enabled: boolean, process?: ProcessType) => void
...
export const useStore = create<AppState>((set, get) => ({
  ...
  showDedupedByProcess: initByProcess<boolean>(false),
  setShowDeduped: (enabled, process) => {
    const key = process ?? get().process
    set((s) => ({
      showDedupedByProcess: {
        ...s.showDedupedByProcess,
        [key]: enabled,
      },
    }))
  },
  ...
  clearReviewState: (process) => {
    ...
    set((s) => ({
      ...
      showDedupedByProcess: { ...s.showDedupedByProcess, [key]: false },
    }))
  },
