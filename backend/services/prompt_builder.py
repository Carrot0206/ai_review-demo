...
11. 【跨文件字段级一致性 · 仅在规则显式声明时触发】：
   - "跨材料数据逻辑校验库"是审核维度名称，不等于申请书与申报模板核对；"跨材料一致性"也可能只是申报模板 JSON 内部跨表/跨字段逻辑。
   - 仅当规则文本、machine_params 或 ai_check_focus 明确写明"申请书/信托文件/信托合同"与"申报模板/模板 JSON"进行一致性核对，且 applicable_materials 同时包含这些材料时，才进行跨文件核对。
   - 跨文件核对要求：两份材料中都能定位到同名字段或同义字段时，才比对取值；issue_location 同时给出两份材料中的真实取值。
   - 若规则只描述模板内字段关系、条件必填、监管口径、日期/金额/枚举/唯一性/字段间逻辑，即便规则名或审核维度含"一致性/跨材料"，也只能在申报模板 JSON 内检查，不得到申请书中查找该字段。
12. 【模板字段的数据来源边界】：
   ...
   - 唯一例外：规则明确要求申请书/信托文件/信托合同与申报模板进行一致性核对时，按第 11 条进行跨文件比对。
13. 【数组/多条记录定位展示】：
   - 申报模板 JSON 中的数组路径使用 0 基下标，例如 "初始受益权信息[0]" 表示第一条记录，"初始受益权信息[1]" 表示第二条记录。
   - 输出 issue_location.location 时必须保留申请材料中的原始数组下标，不要自行改写下标；系统会在展示层统一转换为业务可读的 1 基下标。
   - 表名必须保留完整表名，字段名照常保留。示例：材料路径为 "初始受益权信息[1].受益凭据编号" 时，输出仍为 "初始受益权信息[1].受益凭据编号"。
   - 若问题发生在数组记录中，issue_summary 应包含"第一条"、"第二条"等记录表述，例如"第二条受益权记录业绩比较基准上限低于下限"。
   - 如果原始材料路径没有数组下标，不得自行添加下标。
14. 【缺失材料类规则优先级】：
   - 当问题是"材料未提交/文件缺失"时，优先命中材料提交要求、申请材料完整性、登记必填要素类规则。
   - 例如"缺少信托文件样本"应优先使用"初始登记须提交加盖公章的信托文件样本"类规则。
   - "完整、清晰、可阅读"类规则仅用于材料已经上传但内容不完整、扫描不清晰、无法阅读等质量问题，不作为未提交材料的主规则。
15. 【结构化 AI 规则执行】：
   - 若规则的 rule_text、machine_params 或 ai_check_focus 已写明明确判断条件、计算公式、规范取值或"强校验"，即使依据来源包含"填报说明"，也必须按该结构化口径执行。
   - 对 check_type 为"监管口径符合性"、"语义条件判断"且已给出明确判断口径的规则，不得仅因规则来源文字包含"填报说明"而跳过。
16. 【同一点位命中多规则时的规则事实隔离】
   - 同一材料点位、同一字段或同一组字段可能同时违反多条规则；每条 issue 必须只总结当前 rule_id 对应规则所覆盖的违规事实。
   - issue_summary、issue_location.reason（如有）和 suggestion 不得借用其他 rule_id 的判断口径、规则依据或整改方向。
   - 如果同一取值同时存在多个不同问题，应按不同 rule_id 分别输出，并分别说明各自对应的事实。例如：
     1）若【信托产品全称】含有违法、违规或违背公序良俗的不当字样，应在产品全称规范类规则下总结为"信托产品全称含不当表述"，不得总结为"申报模板与申请书不一致"。
     2）若【信托产品全称】在申报模板与申请书/信托文件中取值不同，只有在当前 rule_id 明确要求跨材料一致性核对时，才总结为"申报模板与申请书/信托文件中产品全称不一致"。
   - 判断当前 issue_summary 是否合格时，先自检：去掉其他材料和其他规则后，仅凭当前 rule_id 的 rule_text、machine_params、ai_check_focus 和当前证据，summary 是否仍然成立；若不成立，不得输出该 summary。
   - 对字段本身内容合规性、字段间逻辑关系、跨材料一致性、材料缺失、材料质量等不同类型问题，必须使用各自规则对应的事实表述，不得混写。
"""
