...
RULE_TAG_TO_MATERIAL_TYPE: dict[str, str] = {
    ...
    "变更登记申报模板JSON": "申报模板",
    "更正登记申报模板JSON": "申报模板",
    "信托产品初始变更更正登记申报模板": "申报模板",
    "预登记产品EXCEL模板.json": "申报模板",
    "申报模板": "申报模板",
    "上一次登记申报模板": "上一次登记申报模板",
    "上一次的初始/变更/更正登记的申请模版": "上一次登记申报模板",
    "上一次的初始/变更/更正登记的申请模板": "上一次登记申报模板",
    ...
    "清算报告": "其他附件",
    "受托人出具的清算报告": "其他附件",
    "证明发生变更事实的文件": "证明发生变更事实的文件",
    "证明发生需要更正事实的文件": "证明发生需要更正事实的文件",
    "其他附件": "其他附件",
...
def _normalize_rule_tag(tag: str) -> Optional[str]:
    ...
    # 兜底：包含关键字
    if "模板" in t:
        if "上一次" in t or "上一笔" in t or "历史登记" in t:
            return "上一次登记申报模板"
        if "原预登记" in t:
            return "原预登记申报模板JSON"
        return "申报模板"
    ...
    if "情况说明" in t or "新型资产服务信托" in t:
        return "新型资产服务信托情况说明"
    if "证明发生需要更正事实" in t or "更正事实证明" in t:
        return "证明发生需要更正事实的文件"
    if "证明发生变更事实" in t or "变更事实证明" in t:
        return "证明发生变更事实的文件"
    return None
