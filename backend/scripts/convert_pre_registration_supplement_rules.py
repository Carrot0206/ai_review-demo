...
from backend.services.schemas import Rule  # noqa: E402


SOURCE = ROOT / "审查规则" / "补充预登记审查规则表.xlsx"
OUTPUT = (
    Path(__file__).resolve().parents[1]
    / "data"
    / "rules"
    / "rules_pre_registration_supplement.json"
)

FILE_SHEET_CANDIDATES = ("文件审查", "文件审查规则")
ELEMENT_SHEET = "要素审查"
...
def first_value(raw: dict[str, str], *names: str) -> str:
    for name in names:
        value = raw.get(name, "")
        if value:
            return value
...
def normalize_dimension(value: str) -> str:
    ...
    if not raw:
        return "其他"
    if raw.endswith("规则库") or raw.endswith("案例库") or raw.endswith("校验库"):
        return raw
    mapping = {
        "格式模板": "格式模板规则库",
        "登记必填要素": "登记必填要素规则库",
...
def risk_level(rule_text: str, dimension: str) -> str:
    ...
    if "审查风险分级" in dimension:
        return "低风险"
    if any(kw in text for kw in ("不得", "严禁", "红线", "违法", "违规")):
        return "高风险"
...
def check_type(rule_text: str, dimension: str) -> str:
    ...
    if "监管" in dimension or "合规" in dimension:
        return "监管口径符合性"
    if any(kw in text for kw in ("枚举", "有效选项", "可选值")):
        return "枚举值"
    return "语义条件判断"
...
def issue_type(rule_text: str, dimension: str, source_sheet: str) -> str:
    ...
    if "审查风险分级" in dimension:
        return "风险提示"
    if "文件" in source_sheet and any(kw in text for kw in ("上传", "提交", "材料", "文件", "附件", "必传")):
        return "材料必交"
    if "签字" in text or "盖章" in text or "公章" in text or "CA认证" in text:
        return "签字盖章"
    if "一致" in text or "相符" in text or "同一" in text:
...
def applicable_materials_for_file(file_name: str, rule_name: str, rule_text: str) -> list[str]:
    ...
    materials: list[str] = []
    if "申请书" in text:
        materials.append("补充预登记申请书")
    if "模板" in text or "JSON" in text or "json" in text:
        materials.append("补充预登记申报模板JSON")
    if "要素报告表" in text:
        materials.append("信托预登记要素报告表")
    if "补充说明" in text or "说明材料" in text:
        materials.append("补充说明材料")
    if "其他附件" in text or "其他文件" in text:
        materials.append("其他附件")
    if not materials:
        materials = ["补充预登记申请书", "补充预登记申报模板JSON", "其他附件"]
    return list(dict.fromkeys(materials))


def applicable_materials_for_element(rule_text: str) -> list[str]:
    text = clean(rule_text)
    materials = ["补充预登记申报模板JSON"]
    if "申请书" in text or "签字" in text or "盖章" in text or "公章" in text:
        materials.append("补充预登记申请书")
    if "要素报告表" in text:
        materials.append("信托预登记要素报告表")
    if "补充说明" in text or "说明材料" in text:
        materials.append("补充说明材料")
    if "附件" in text or "证明材料" in text:
        materials.append("其他附件")
    return list(dict.fromkeys(materials))
...
def skip_when(rule_text: str, table_name: str, field_name: str, source_sheet: str) -> str:
    text = clean(rule_text)
    if "系统状态" in text or "办理状态" in text or "中信登系统" in text:
        return "系统状态 unknown"
    if "文件" in source_sheet:
        return ""
...
def ai_focus(rule_text: str, source_sheet: str) -> list[str]:
    if "文件" in source_sheet:
        return [
            "判断补充预登记申请材料是否齐备、形式是否符合要求",
            "结合申请书、申报模板 JSON、要素报告表、补充说明材料和其他附件识别缺失、不一致或证明不充分的问题",
            "输出问题时定位到具体材料、页码、字段或文本片段",
        ]
    return [
        "优先在补充预登记申报模板 JSON 的对应表和字段中判断规则是否命中",
        "申请书、要素报告表、补充说明材料和其他附件不得替代申报模板字段值",
        "输出问题时定位到具体表名、记录和字段",
    ]
...
def file_review_rules(ws) -> list[dict[str, Any]]:
    ...
    for row_no, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
        ...
        if clean(raw.get("分类", "")).lower() != "ai":
            continue
        file_name = first_value(raw, "文件名", "材料名称", "材料")
        rule_name = first_value(raw, "文件审查规则", "规则名称", "审查规则") or file_name
        rule_text = first_value(raw, "具体的规则", "具体规则", "规则内容", "规则")
        if not rule_text:
            continue
        dim = normalize_dimension(first_value(raw, "审核维度", "审查维度"))
        basis_file, basis_text = split_basis(first_value(raw, "审核依据", "审查依据", "依据"))
        rid = f"SUPP-FILE-AI-{len(rules) + 1:03d}"
        rules.append(
            {
                "rule_id": rid,
                "registration_type": "补充预登记",
                "rule_source": "内置规则",
                "source_file": SOURCE.name,
                "source_sheet": ws.title,
                "source_row": row_no,
                "rule_name": rule_name or "补充预登记文件审查",
                "rule_text": rule_text,
                ...
                "machine_params": "source_classification=ai",
...
def element_ai_rules(ws) -> list[dict[str, Any]]:
    ...
    for row_no, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
        raw = row_dict(headers, row)
        if clean(raw.get("分类结果", "")) != ELEMENT_AI_MARK:
            continue
        table_name = normalize_table_name(first_value(raw, "要素分类", "表名", "所属表"))
        field_name = first_value(raw, "要素名", "字段名", "字段")
        if field_name == table_name:
            field_name = ""
        rule_text = first_value(raw, "具体规则(已融合全部枚举值)", "具体规则", "规则内容", "规则")
        if not rule_text:
            continue
        dim = normalize_dimension(first_value(raw, "审查维度", "审核维度"))
        basis_file, basis_text = split_basis(first_value(raw, "审查依据", "审核依据", "依据"))
        rid = f"SUPP-ELEMENT-AI-{len(rules) + 1:03d}"
        rules.append(
            {
                "rule_id": rid,
                "registration_type": "补充预登记",
                ...
                "source_sheet": ws.title,
                "source_row": row_no,
                "rule_name": field_name or table_name or "补充预登记要素审查",
                "rule_text": rule_text,
                "basis_file": basis_file,
                "basis_text": basis_text,
                "review_dimension": dim,
                ...
                "check_type": check_type(rule_text, dim),
                "trigger_condition": "",
                "machine_params": f"source_classification={ELEMENT_AI_MARK}",
                "risk_level": risk_level(rule_text, dim),
                "ai_check_focus": ai_focus(rule_text, ws.title),
...
def validate_rules(rules: list[dict[str, Any]]) -> None:
    ...
    for rule in rules:
        ...
        if not (
            rule["rule_id"].startswith("SUPP-FILE-AI-")
            or rule["rule_id"].startswith("SUPP-ELEMENT-AI-")
        ):
            raise RuntimeError(f"非法补充预登记规则 ID: {rule['rule_id']}")
        if rule["source_sheet"] in FILE_SHEET_CANDIDATES:
            if "source_classification=ai" not in rule.get("machine_params", ""):
                raise RuntimeError(f"{rule['rule_id']} 文件审查规则未标记 ai 来源")
        if rule["source_sheet"] == ELEMENT_SHEET:
            if ELEMENT_AI_MARK not in rule.get("machine_params", ""):
                raise RuntimeError(f"{rule['rule_id']} 要素审查规则未标记 AI 推理来源")


def resolve_file_sheet(wb) -> str:
    for name in FILE_SHEET_CANDIDATES:
        if name in wb.sheetnames:
            return name
    raise RuntimeError(f"未找到文件审查 sheet，候选名称: {', '.join(FILE_SHEET_CANDIDATES)}")


def main() -> None:
    if not SOURCE.exists():
        raise FileNotFoundError(f"未找到补充预登记规则源文件: {SOURCE}")
    wb = load_workbook(SOURCE, data_only=True, read_only=True)
    file_sheet = resolve_file_sheet(wb)
    if ELEMENT_SHEET not in wb.sheetnames:
        raise RuntimeError(f"未找到要素审查 sheet: {ELEMENT_SHEET}")
    rules = file_review_rules(wb[file_sheet]) + element_ai_rules(wb[ELEMENT_SHEET])
    validate_rules(rules)
    payload = {
        "rule_set_id": "pre_registration_supplement_ai_rules_v1",
        "registration_type": "补充预登记",
        "source_file": str(SOURCE.relative_to(ROOT)),
        "description": "补充预登记 AI 审核内置规则；仅来源于审查规则文件夹《补充预登记审查规则表.xlsx》中，文件审查/文件审查规则 sheet 标记为 ai 的规则，以及要素审查 sheet 分类结果为需借助AI推理判断的规则。",
