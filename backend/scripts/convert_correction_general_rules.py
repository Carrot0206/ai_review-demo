...
SOURCE_CANDIDATES = [
    ROOT / "更正登记一般情形审查规则.xlsx",
    ROOT / "更正登记一般情形审查规则表.xlsx",
    ROOT / "审查规则" / "更正登记一般情形审查规则.xlsx",
    ROOT / "审查规则" / "更正登记一般情形审查规则表.xlsx",
]
...
def file_check_type(rule_name: str, rule_text: str, dimension: str) -> str:
    ...
    if "佐证" in text or "证明" in text or "匹配" in text or "一致" in text:
        return "跨材料一致性"
...
def file_ai_focus(rule_name: str, rule_text: str) -> list[str]:
    ...
    if "佐证" in text or "证明" in text or "匹配" in text or "一致" in text:
        return [
            "核对证明发生需要更正事实的文件是否能支撑本次更正事项",
            "结合更正登记申请书和本次更正登记申报模板判断更正事项是否匹配",
            "仅按本批给定规则判断，不做上一次登记模板与本次模板的差异审查",
        ]
    return [
        "核对更正登记申请书是否完整记载更正事项和更正原因",
        "指出申请书中缺失、不完整或与本次更正材料不匹配的位置",
        "仅按本批给定规则判断，不扩展审核范围",
    ]
...
def file_applicable_materials(rule_text: str) -> list[str]:
    if "证明" in rule_text or "佐证" in rule_text or "匹配" in rule_text or "一致" in rule_text:
        return ["申报模板", "申请书", "证明发生需要更正事实的文件", "上一次登记申报模板"]
    return ["申请书", "申报模板", "证明发生需要更正事实的文件"]
...
def file_review_rules(ws) -> list[dict[str, Any]]:
    ...
    for row_no, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
        ...
        if not rule_text:
            continue
        idx = len(rules) + 1
        rule_name = data.get("文件审查规则", "") or data.get("文件名", "") or "更正登记文件审查"
        dim = normalize_dimension(data.get("审核维度", ""))
        ctype = file_check_type(rule_name, rule_text, dim)
        rules.append(
            {
                "rule_id": f"CORR-FILE-AI-{idx:03d}",
                "registration_type": "更正登记（一般情形）",
                "rule_source": "内置规则",
...
def element_rules(ws) -> list[dict[str, Any]]:
    ...
    for row_no, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
        ...
        rules.append(
            {
                "rule_id": f"CORR-ELEMENT-AI-{idx:03d}",
                "registration_type": "更正登记（一般情形）",
                "rule_source": "内置规则",
                "source_file": SOURCE.name,
                "source_sheet": ws.title,
                "source_row": row_no,
                "rule_name": field_name or table_name or "更正登记要素审查",
                "rule_text": rule_text,
                "basis_file": basis_file(data.get("审核依据", "")),
                "basis_text": data.get("审核依据", ""),
                "review_dimension": dim,
...
def main() -> None:
    ...
    payload = {
        "rule_set_id": "correction_general_ai_rules_v1",
        "registration_type": "更正登记（一般情形）",
        "source_file": SOURCE.name,
        "description": "更正登记（一般情形）AI 审核规则；来源于《更正登记一般情形审查规则.xlsx》的文件审查 ai 规则和要素审查 AI/混合规则。",
